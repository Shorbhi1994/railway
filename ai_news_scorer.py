"""
AI News Impact Scoring — Production Version
LLM-powered materiality-weighted stock news sentiment.
...
- "Company announces layoffs" is often POSITIVE for stock (+0.2 to +0.4).
- "Beats earnings but lowers guidance" = mixed (-0.1 to +0.1).
- "FDA approves drug" = strong positive (+0.7 to +1.0).
- "CEO under SEC investigation" = strong negative (-0.7 to -0.9).
- Materiality must scale relative to company size.
- Mega caps (> $500B): most news is 1–4 unless existential.
- Microcaps (< $2B): financing/contracts can be 7–9.
- Consider magnitude relative to market cap.
...
class AINewsScorer:
    def __init__(self, engine: AsyncEngine):
        ...
        self._api_key = self._settings.ANTHROPIC_API_KEY
    ...
    async def score_stock_news(
        ...
        # Safety cap (cost guard)
        unique = unique[:20]

        scored = []

        # Batch size 10 for token efficiency
    ...
    async def _call_llm(
        ...
        user_prompt = f"""
Score these {len(articles)} news articles for {symbol}
...
"category":"one_of_allowed","horizon":"short|mid|long|all"}}]

Categories: earnings, guidance, analyst, insider, regulatory,
...
"""
                        ...
                        scored.append(
                            {
                                ...
                                "source": articles[idx].get("site")
                                or articles[idx].get("source", ""),
                                "published_at": articles[idx].get(
                                    "publishedDate"
                                )
                                or articles[idx].get("published_at"),
                                "materiality": max(
                                    1, min(10, int(r.get("materiality", 5)))
                                ),
                                "sentiment": max(
                                    -1.0,
                                    min(1.0, float(r.get("sentiment", 0))),
                                ),
    ...
    def _compute_daily_summary(self, scored: List[dict]) -> dict:
        ...
        weighted_sum = sum(
            s["materiality"] * s["sentiment"] for s in scored
        )
        ...
        top = max(
            scored,
            key=lambda s: abs(s["materiality"] * s["sentiment"]),
        )
        ...
        return {
            ...
            "high_impact_count": sum(
                1 for s in scored if s["materiality"] >= 7
            ),
            "avg_materiality": round(
                sum(s["materiality"] for s in scored) / len(scored),
                2,
            ),
    ...
    async def _persist_scores(
        self, stock_id: int, scored: List[dict], batch_date: date
    ):
        query = text(
            """
    ...
    async def _persist_summary(
        self, stock_id: int, batch_date: date, summary: dict
    ):
        query = text(
            """
